...
def clear():
    """Clear all in-memory alerts and reset correlation state.
    The on-disk alerts.log is preserved (rename or delete it manually
    if you want a clean log file too)."""
    global last_correlated_time
    cleared = len(alerts)
    alerts.clear()
    last_seen.clear()
    last_correlated_time = None
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] CLEARED {cleared} alerts")
    # Redirect back to the dashboard so the button works as a normal form post.
...
def status_page():
    ...
    <body>
      ...
      <div style="display:flex; align-items:center; gap:1em; margin-top:0.5em;">
        <p style="margin:0;">Total alerts received: <b>{{ count }}</b></p>
        <form action="/clear" method="post" style="margin:0;"
              onsubmit="return confirm('Clear the dashboard? (alerts.log on disk is preserved.)');">
          <button type="submit"
                  style="background:#c0392b; color:white; border:0; padding:0.4em 0.9em;
